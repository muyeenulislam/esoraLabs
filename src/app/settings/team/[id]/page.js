  ...
  const columns = [
    {
      title: "Project Name",
      dataIndex: "services",
      render: (text, record) => <div>{record?.services}</div>,
    },
    {
      title: "Priority",
      dataIndex: "priority",
      render: (text, record) => (
        <StatusIndicator text={record?.priority || "N/A"} className="w-max" />
      ),
    },
    {
      title: "Status",
      dataIndex: "Status",
      render: (text, record) => (
        <StatusIndicator text={record?.status || "N/A"} className="w-max" />
      ),
    },
    {
      title: "Assigned At",
      dataIndex: "assignedon",
      render: (text, record) => <div>{record?.assingedOn}</div>,
    },
    {
      title: "Due By",
      dataIndex: "dueDtae",
      render: (text, record) => <div>{record?.dueDate}</div>,
    },
    {
      title: " ",
      dataIndex: "action",
      ...
      <div className="w-[90%] lg:w-[70%] m-auto flex flex-col rounded-xl shadow-md">
        ...
        <div className="flex p-[24px] bg-[#F9FAFB]  flex-col">
          ...
          <div className="border border-gray-200 shadow-clientCard rounded-2xl">
            <TableWithoutCheckbox columns={columns} data={data?.projects} />
